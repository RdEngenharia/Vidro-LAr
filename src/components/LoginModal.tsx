...
import { checkCnpjAvailable } from '../lib/cnpjApi';
import { Building2, Lock, Mail, ShieldCheck, ArrowRight, WifiOff, User, Gift, Search, Check, Loader2, AlertTriangle } from 'lucide-react';
...
//
// O CNPJ é consultado e validado DE VERDADE na Receita Federal já nesta tela
// — antes da conta existir — e fica travado depois de criado (não editável),
// pra dificultar fraude (ex: abrir vários testes grátis com CNPJs fictícios).
//
...
export const LoginModal: React.FC<LoginModalProps> = ({ isOpen }) => {
  ...
  // Estado da consulta/validação do CNPJ — a conta só pode ser criada depois
  // de "valid" (razão social confirmada na Receita Federal).
  const [cnpjStatus, setCnpjStatus] = useState<'idle' | 'checking' | 'valid' | 'invalid'>('idle');
  const [cnpjRazaoSocial, setCnpjRazaoSocial] = useState('');
  const [cnpjError, setCnpjError] = useState('');
  const [cnpjValidatedValue, setCnpjValidatedValue] = useState('');
  ...
  const handleCheckCnpj = async () => {
    ...
    try {
      const result = await checkCnpjAvailable(cnpj);
      setCnpjStatus('valid');
      setCnpjRazaoSocial(result.razaoSocial);
      setCnpjValidatedValue(cnpj);
      // Preenche o nome da empresa automaticamente com a razão social oficial,
      // se a pessoa ainda não tiver digitado nada.
      if (!companyName.trim()) {
        setCompanyName(result.nomeFantasia || result.razaoSocial);
      }
    } catch (err: any) {
      setCnpjStatus('invalid');
      setCnpjRazaoSocial('');
      setCnpjError(err?.message || 'Não foi possível validar este CNPJ.');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'register') {
      ...
      // O CNPJ precisa ter sido validado com sucesso, e não pode ter sido
      // alterado depois da validação (senão a razão social confirmada não
      // corresponde mais ao número que seria enviado).
      if (cnpjStatus !== 'valid' || cnpj !== cnpjValidatedValue) {
        setErrorMessage('Consulte e confirme o CNPJ antes de continuar.');
        return;
      }
            ...
            {mode === 'register' && (
              <>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        // Qualquer alteração no número invalida uma consulta
                        // anterior — evita enviar um CNPJ diferente do que
                        // foi de fato confirmado na Receita.
                        if (cnpjStatus !== 'idle') {
                          setCnpjStatus('idle');
                          setCnpjRazaoSocial('');
                        }
                      ...
                      className="flex-1 min-w-0 px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-slate-900"
                    />
                    <button
                      type="button"
                      onClick={handleCheckCnpj}
                      disabled={cnpjStatus === 'checking'}
                      className="shrink-0 px-3 py-2 bg-slate-900 hover:bg-slate-800 disabled:opacity-60 text-white text-xs font-bold rounded-xl transition-colors cursor-pointer flex items-center gap-1.5"
                    >
                      {cnpjStatus === 'checking' ? (
                  ...
                  {cnpjStatus === 'valid' && (
                    <p className="mt-1.5 text-[11px] text-emerald-700 font-semibold flex items-start gap-1">
                      <Check className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                      {cnpjRazaoSocial}
                    </p>
                  )}
                  {cnpjStatus === 'invalid' && cnpjError && (
                    <p className="mt-1.5 text-[11px] text-rose-600 font-semibold flex items-start gap-1">
                      <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                      {cnpjError}
                    </p>
                  )}
                  <p className="mt-1 text-[10px] text-slate-400">
                    Consultado direto na Receita Federal e travado após o cadastro — não pode ser alterado depois.
                  </p>
                </div>
              </>
            )}
